...
class TestComputeInvoicePolicy(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.grp_po_manager = self.env.ref("account.group_account_manager")
    ...
    def _create_user_1(self):
        val = {
            ...
            "notify_email": "none",
            "groups_id": [(6, 0, [self.group_employee_id, self.grp_po_manager.id])],
        }
    ...
    def test_compute_case_admin(self):
        ...
        self.assertEqual(True, invoice.restart_ok)
        self.assertEqual(True, invoice.reopen_ok)
    ...
    def test_compute_case_1(self):
        ...
        invoice.journal_id.invoice_open_group_ids = [(6, 0, [self.grp_open.id])]

        invoice.journal_id.invoice_refund_group_ids = [(6, 0, [self.grp_refund.id])]

        invoice.journal_id.invoice_cancel_group_ids = [(6, 0, [self.grp_cancel.id])]
        ...
        self.assertEqual(False, invoice.sudo(self.user_1.id).open_ok)
        self.assertEqual(False, invoice.sudo(self.user_1.id).refund_ok)
        self.assertEqual(False, invoice.sudo(self.user_1.id).cancel_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).restart_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).reopen_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).send_email_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).proforma_ok)

        # Condition :
        #   1. User 1 have group
        # Add Group Validate
        self.user_1.groups_id = [(4, self.grp_open.id)]
        # Add Group Refund
        self.user_1.groups_id = [(4, self.grp_refund.id)]
        # Add Group Cancel
        self.user_1.groups_id = [(4, self.grp_cancel.id)]

        # Result
        #   1. User 1 can Validate
        #   2. User 1 can Refund
        #   3. User 1 can Cancel
        self.assertEqual(True, invoice.sudo(self.user_1.id).open_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).refund_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).cancel_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).restart_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).reopen_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).send_email_ok)
        self.assertEqual(True, invoice.sudo(self.user_1.id).proforma_ok)
    ...
    def test_compute_case_2(self):
        ...
        invoice.journal_id.invoice_restart_group_ids = [
            (6, 0, [self.grp_set_to_draft.id])
        ]

        invoice.journal_id.invoice_reopen_group_ids = [(6, 0, [self.grp_re_open.id])]

        # Result
        #   1. User 2 cannot Set To Draft
        #   2. User 2 cannot Re-Open

        self.assertEqual(True, invoice.sudo(self.user_2.id).open_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).refund_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).cancel_ok)
        self.assertEqual(False, invoice.sudo(self.user_2.id).restart_ok)
        self.assertEqual(False, invoice.sudo(self.user_2.id).reopen_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).send_email_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).proforma_ok)

        # Condition :
        #   1. User 2 have group
        # Add Group Set To Draft
        self.user_2.groups_id = [(4, self.grp_set_to_draft.id)]
        # Add Group Re-Open
        self.user_2.groups_id = [(4, self.grp_re_open.id)]
        ...
        self.assertEqual(True, invoice.sudo(self.user_2.id).refund_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).cancel_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).restart_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).reopen_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).send_email_ok)
        self.assertEqual(True, invoice.sudo(self.user_2.id).proforma_ok)
    ...
    def test_compute_case_3(self):
        ...
        invoice.journal_id.invoice_send_email_group_ids = [
            (6, 0, [self.grp_send_email.id])
        ]

        invoice.journal_id.invoice_proforma_group_ids = [(6, 0, [self.grp_proforma.id])]

        # Result
        #   1. User 3 cannot Send by Email
        #   2. User 3 cannot PRO-FORMA

        self.assertEqual(True, invoice.sudo(self.user_3.id).open_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).refund_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).cancel_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).restart_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).restart_ok)
        self.assertEqual(False, invoice.sudo(self.user_3.id).send_email_ok)
        self.assertEqual(False, invoice.sudo(self.user_3.id).proforma_ok)

        # Condition :
        #   1. User 3 have group
        # Add Group Send by Email
        self.user_3.groups_id = [(4, self.grp_send_email.id)]
        # Add Group PRO-FORMA
        self.user_3.groups_id = [(4, self.grp_proforma.id)]
        ...
        self.assertEqual(True, invoice.sudo(self.user_3.id).cancel_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).restart_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).reopen_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).send_email_ok)
        self.assertEqual(True, invoice.sudo(self.user_3.id).proforma_ok)
